...
const Chatbot: React.FC<ChatbotProps> = ({ lang }) => {
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    try {
      ...
      let responseContent = generateFallbackResponse(userMessage.content);
      let responseSources = [t('chatbot.fallback_source')];
      let fetchedData: RAGResponse | null = null;

      if (response.ok) {
        const data: RAGResponse = await response.json();
        fetchedData = data;
        responseContent = data.answer || responseContent;
        responseSources = data.sources.length > 0 ? data.sources.map(s => s.source) : responseSources;
      ...
      toast({
        ...
        description: fetchedData?.retrieved_chunks.length > 0
          ? t('chatbot.response_desc_with_sources', { count: fetchedData?.retrieved_chunks.length, latency: fetchedData?.latency_ms })
          : t('chatbot.response_desc_fallback'),
      });

    } catch (err: any) {
      console.error('Chat error:', err);
      const responseContent = generateFallbackResponse(userMessage.content);
      const assistantMessage: ChatMessage = {
        role: 'assistant',
        content: responseContent,
        sources: [t('chatbot.fallback_source')],
        timestamp: new Date().toISOString()
      };
      setMessages(prev => [...prev, assistantMessage]);
