...
class GenerationService:
    ...
    def _initialize(self):
        ...
        try:
            if not settings.gemini_api_key or settings.gemini_api_key.startswith("replace-with"):
                raise ValueError("GEMINI_API_KEY is required and must be set in backend/.env.")
...
Answer the question naturally and helpfully:"""
    ...
    def generate_answer(
        ...
        if not self.gemini_model:
            raise RuntimeError(
                ...
                "Set GEMINI_API_KEY and a valid GEMINI_MODEL in backend/.env."
            )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.initialized and self.gemini_model:
                return {
                    "status": "running",
                    "provider": "gemini",
                    "model": settings.gemini_model
