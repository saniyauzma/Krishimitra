...
USE_WEATHER_API = bool(API_KEY and not API_KEY.startswith("replace-with"))
BASE_URL = "https://api.openweathermap.org/data/2.5/weather"


def _get_fallback_weather(city: str, state: str = None, country: str = "IN") -> dict:
    logger.warning(
        "OPENWEATHER_API_KEY is missing or weather lookup failed. "
        "Returning fallback weather data."
    )
    return {
        "temperature": 28.0,
        "humidity": 65,
...
def get_weather_by_location(city: str, state: str = None, country: str = "IN") -> dict:
    ...
    if not USE_WEATHER_API:
        return _get_fallback_weather(city, state, country)
    ...
    try:
        ...
        logger.warning(f"Weather API request failed for {location_query}: {exc}")
        return _get_fallback_weather(city, state, country)
